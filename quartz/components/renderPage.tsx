...
export function renderPage(
  ...
  // The dev server serves from the site root unless --baseDir is given (used to
  // preview a project-pages subpath locally). basePath must match so client-side
  // components (explorer, spa router) keep links under the subpath.
  let basePath = ""
  if (cfg.baseUrl && !componentData.ctx.argv.serve) {
    basePath = new URL(`https://${cfg.baseUrl}`).pathname.replace(/\/$/, "")
  } else if (componentData.ctx.argv.serve && componentData.ctx.argv.baseDir) {
    basePath = `/${componentData.ctx.argv.baseDir.replace(/(^\/)|(\/$)/g, "")}`
  }
  const doc = (
