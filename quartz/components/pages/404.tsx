...
const NotFound: QuartzComponent = ({ cfg, ctx }: QuartzComponentProps) => {
  ...
  let baseDir = ctx.argv.serve ? "/" : url.pathname
  // In serve mode with --baseDir (subpath preview), the home link must stay
  // under the previewed prefix.
  if (ctx.argv.serve && ctx.argv.baseDir) {
    baseDir = `/${ctx.argv.baseDir.replace(/(^\/)|(\/$)/g, "")}/`
  }
